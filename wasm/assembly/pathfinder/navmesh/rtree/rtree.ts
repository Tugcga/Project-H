...
export class RTree extends Serializable {
    ...
    project_vector_to_line(start_x: f32, start_y: f32, finish_x: f32, finish_y: f32, pos_x: f32, pos_y: f32, out_target: Vector2, ignore_back_side: bool): void {
        ...
        var closed_polygon_index: i32 = -1;
        var closed_polygon_line_index: i32 = -1;
                    ...
                    if (is_process) {
                        ...
                        if (Mathf.abs(denum) >= Mathf.abs(num)) {
                            ...
                            if (d < 0.0) {
                                ...
                                if (t >= 0.0 && t < closed_t) {
                                    closed_t = t;
                                    closed_polygon_index = i;
                                    closed_polygon_line_index = line_index;
                                }
                            }
                        }
                    }
        ...
        if (closed_polygon_index >= 0 && closed_polygon_line_index >= 0 && closed_t < 1.0) {
            // get line
            const polygon = polygons[closed_polygon_index];
            const polygon_coordinates = polygon.coordinates();
            const coordinates_count = polygon_coordinates.length;
            const line_start_x = polygon_coordinates[2*closed_polygon_line_index];
            const line_start_y = polygon_coordinates[2*closed_polygon_line_index + 1];
            const line_end_x = polygon_coordinates[(2*closed_polygon_line_index + 2) % coordinates_count];
            const line_end_y = polygon_coordinates[(2*closed_polygon_line_index + 3) % coordinates_count];

            // we shold project input vector into the line and output this projected vector
            // calculate normalized edge-vector
            const line_vector_x = line_end_x - line_start_x;
            const line_vector_y = line_end_y - line_start_y;
            const line_vector_length = Mathf.sqrt(line_vector_x * line_vector_x + line_vector_y * line_vector_y);
            if (line_vector_length > EPSILON) {
                const line_vector_normalized_x = line_vector_x / line_vector_length;
                const line_vector_normalized_y = line_vector_y / line_vector_length;

                // also normalize the input vector
                const input_vector_x = finish_x - start_x;
                const input_vector_y = finish_y - start_y;
                const input_vector_length = Mathf.sqrt(input_vector_x * input_vector_x + input_vector_y * input_vector_y);

                if (input_vector_length > EPSILON) {
                    const input_vector_normalized_x = input_vector_x / input_vector_length;
                    const input_vector_normalized_y = input_vector_y / input_vector_length;

                    // calculate dot product - this is cos of the angle
                    const cos_angle = input_vector_normalized_x * line_vector_normalized_x + input_vector_normalized_y * line_vector_normalized_y;

                    // calculate the length of the input vector after intersection point
                    const inner_x = input_vector_x * (1.0 - closed_t);
                    const inner_y = input_vector_y * (1.0 - closed_t);
                    const inner_length = Mathf.sqrt(inner_x*inner_x + inner_y*inner_y);

                    // get vector, collinear to the line, but with length euqal to cos_angle * inner_length
                    const line_collinear_x = line_vector_normalized_x * cos_angle * inner_length;
                    const line_collinear_y = line_vector_normalized_y * cos_angle * inner_length;

                    // calcualte projector vector (from input endpoint to the line)
                    const projector_x: f32 = -1.0 * inner_x + line_collinear_x;
                    const projector_y: f32 = -1.0 * inner_y + line_collinear_y;

                    // finally add this projector to the input vector
                    const shifted_target_x = finish_x + projector_x * 1.0;
                    const shifted_target_y = finish_y + projector_y * 1.0;
                    // we should check is this vector intersects with other edges
                    // this is repeat of the previous code
                    // -----------------------------------
                    const new_edge_rect = new Rectangle(Mathf.min(start_x, shifted_target_x), Mathf.max(start_y, shifted_target_y),
                                                        Mathf.max(start_x, shifted_target_x), Mathf.min(start_y, shifted_target_y));
                    const new_polygons = this.range_search(edge_rect);
                    const new_poly_count = polygons.length;
                    var new_closed_t: f32 = 1.0;
                    var new_closed_polygon_index: i32 = -1;
                    var new_closed_polygon_line_index: i32 = -1;
                    for (let i = 0; i < new_poly_count; i++) {
                        const polygon = new_polygons[i];
                        const corners = polygon.corners();
                        if (corners > 1) {
                            const lines_count = corners == 2 ? 1 : corners;
                            const polygon_coordinates = polygon.coordinates();
                            ...
                            for (let line_index = 0; line_index < lines_count; line_index++) {
                                ...
                                const denum = (shifted_target_x - start_x) * in_edge_y - (shifted_target_y - start_y) * in_edge_x;
                                ...
                                if (Mathf.abs(denum) >= Mathf.abs(num)) {
                                    ...
                                    // calc the point in the edge
                                    const p_x = start_x + t * (shifted_target_x - start_x);
                                    const p_y = start_y + t * (shifted_target_y - start_y);
                                    ...
                                    if (d < 0.0) {
                                        // point inside the interval
                                        if (t >= 0.0 && t < new_closed_t) {
                                            new_closed_t = t;
                                            new_closed_polygon_index = i;
                                            new_closed_polygon_line_index = line_index;
                                        }
                                    }
                                }
                            }
                    ...
                    // --------------------------------
                    // end repeat
                    if (new_closed_polygon_index >= 0 && (new_closed_polygon_index != closed_polygon_index || new_closed_polygon_line_index != closed_polygon_line_index)) {
                        // there is intersection, nothing to move
                        out_target.set_values(pos_x, pos_y);
                    } else {
                        // no additional intersection
                        out_target.set_values(shifted_target_x, shifted_target_y);
                    }
                } else {
                    // input vector very short
                    out_target.set_values(finish_x, finish_y);
                }
            } else {
                // this is very short edge, ignore it
                out_target.set_values(finish_x, finish_y);
            }
        } else {
            // no intersections
