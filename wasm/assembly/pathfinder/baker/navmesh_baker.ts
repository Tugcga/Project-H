...
export class NavmeshBaker {
    ...
    get_navmesh_vertices_f32(): StaticArray<f32> {
        const length = this.m_output_vertices.length;
        let to_return: StaticArray<f32> = new StaticArray<f32>(length);
        for(let i = 0; i < length; i++) {
            to_return[i] = <f32>this.m_output_vertices[i];
        }
        return to_return;
    }
