...
export class PathFinder{
    private m_navmesh: Navmesh | null = null;
    ...
    constructor(vertices: StaticArray<f32> | null, 
        ...
        let simulators = new StaticArray<RVOSimulator>(groups_count);
        ...
        let agents_group_id = new List<List<i32>>();
    ...
    set_agent_destination(agent_id: i32, position_x: f32, position_y: f32, position_z: f32): boolean{
        let is_assign = true;
        let x = position_x;
        let y = position_y;
        let z = position_z;
    ...
    get_agent_destination(agent_id: i32): Float32Array {
        ...
        if(this.m_agents_activity[agent_inner_index]) {
            const path_coordinates = this.m_agents_path[agent_inner_index];
            const path_coordinates_count = path_coordinates.length;
            // get last path points
            if(path_coordinates_count > 0) {
                const to_return = new Float32Array(3);
                to_return[0] = path_coordinates[path_coordinates_count - 3];
                to_return[1] = path_coordinates[path_coordinates_count - 2];
                to_return[2] = path_coordinates[path_coordinates_count - 1];
                return to_return;
            }
        }
    ...
    @inline
    set_agent_position(agent_id: i32, x: f32, y: f32, z: f32): void {
        const agent_inner_index = this._get_agent_inner_index(agent_id);
        if(agent_inner_index > -1){
            const group_index = this.m_agents_group[agent_inner_index];
            let sim = unchecked(this.m_simulators[group_index]);
            sim.set_agent_position(agent_inner_index, x, z);
        }
    }
